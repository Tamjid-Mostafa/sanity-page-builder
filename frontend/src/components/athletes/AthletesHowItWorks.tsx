...
interface AthletesHowItWorksProps {
  studyRoomImage: SanityImageField;
  tabletLearningImage: SanityImageField;
  classroomImage: SanityImageField;
}
...
export function AthletesHowItWorks({
  studyRoomImage: studyRoomData,
  tabletLearningImage: tabletLearningData,
  classroomImage: classroomData,
}: AthletesHowItWorksProps) {
  const classroomImg = urlForImage(classroomData, { maxWidth: 900 });
  const studyRoomImg = urlForImage(studyRoomData, { maxWidth: 600 });
  const tabletImg = urlForImage(tabletLearningData, { maxWidth: 600 });
    ...
    <section className="py-14 md:py-24 bg-background">
      <div className="container mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] items-start">
          ...
          >
            {classroomImg && (
              <div className="relative col-span-2 aspect-16/10 rounded-xl overflow-hidden border border-border">
                <Image
                  src={classroomImg.src}
                  alt={classroomImg.alt ?? "Students learning in a classroom environment"}
                  fill
                  className="object-cover"
                  sizes="(max-width: 1024px) 100vw, 35vw"
                  placeholder={classroomImg.blurDataURL ? "blur" : "empty"}
                  blurDataURL={classroomImg.blurDataURL ?? undefined}
                />
              </div>
            )}
            {studyRoomImg && (
              <div className="relative aspect-square rounded-xl overflow-hidden border border-border">
                <Image
                  src={studyRoomImg.src}
                  alt={studyRoomImg.alt ?? "Focused students studying together"}
                  fill
                  className="object-cover"
                  sizes="(max-width: 1024px) 50vw, 17vw"
                  placeholder={studyRoomImg.blurDataURL ? "blur" : "empty"}
                  blurDataURL={studyRoomImg.blurDataURL ?? undefined}
                />
              </div>
            )}
            {tabletImg && (
              <div className="relative aspect-square rounded-xl overflow-hidden border border-border">
                <Image
                  src={tabletImg.src}
                  alt={tabletImg.alt ?? "Student using tablet for flexible learning"}
                  fill
                  className="object-cover"
                  sizes="(max-width: 1024px) 50vw, 17vw"
                  placeholder={tabletImg.blurDataURL ? "blur" : "empty"}
                  blurDataURL={tabletImg.blurDataURL ?? undefined}
                />
              </div>
            )}
          </motion.div>
